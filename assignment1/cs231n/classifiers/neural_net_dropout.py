...
class TwoLayerNetDropout(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # Generate drop out mask for W1 and W2
    P = np.random.rand(*H.shape) < self.p
    P /= self.p
    ...
    loss = np.sum(correct_prob) / N + reg * (np.sum(W1 * W1) + np.sum(W2 * W2))
    ...
    dW2 = np.matmul(H.T, dscores) + reg * 2.0 * W2

    dhidden = np.matmul(dscores, W2.T)
    dhidden[H <= 0] = 0

    db1 = np.sum(dhidden, axis = 0, keepdims = False)
    dW1 = np.matmul(X.T, dhidden) + reg * 2.0 * W1
